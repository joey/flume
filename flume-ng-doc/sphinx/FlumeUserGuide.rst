...
kite.syncable.commitOnBatch  true     If true, the writer will be synced and the Flume transaction
                                      ...
                                      DatasetReaders.
kite.syncIfAvailable         true     Controls whether the sink will sync data when committing
                                      the transaction or if the sink will only flush data. Syncing
                                      gaurentees that data will be written on stable storage on
                                      the remote system while flushing only gaurentees that data
                                      has left Flume's client buffers. Some Datasets (Parquet
                                      formatted Datasets and HBase) don't support sync in which
                                      case only flush will be used.
